...
# Baken-Konfiguration (Aktuell auf 30 Sekunden für deinen Test)
...
# --- DYNAMISCHE PTT & AUDIO LOGIK ---
def play_text_to_radio(text, msg_id):
    global last_radio_activity
    print(f"\n📢 VERARBEITE [ID: 0x{msg_id}]: {text}")
    
    # Der Timer wird erst JETZT zurückgesetzt, da das Gerät real sendet
    last_radio_activity = time.time()
    ...
    try:
        ...
        print(f"📡 Öffne PTT-Port {PTT_PORT}...")
        try:
            ser_ptt = serial.Serial(PTT_PORT, 9600, timeout=1)
            ser_ptt.setRTS(True)
            ser_ptt.setDTR(True)
        except Exception as e:
            print(f"❌ PTT Hardware-Fehler auf {PTT_PORT}: {e}")
            return
...
# Hintergrund-Thread für die Bake (Entkoppelt vom MQTT-Verkehr)
def beacon_worker():
    global last_radio_activity
    print("🛸 Baken-Überwachung gestartet.")
    while True:
        time.sleep(1)  # Kurzer Prüfzyklus für den Testlauf
        now = time.time()
        
        if now - last_radio_activity >= BAKEN_INTERVAL_SEC:
            # Setze den Timer sofort hoch, damit die Bake nicht in eine Dauerschleife läuft
...
if __name__ == "__main__":
    ...
    if args.ip:
        ...
        try:
            ...
            print(f"🚀 Gateway läuft und überwacht Baken-Intervall auf {args.ip}...")
            print("-" * 55)
            client.loop_forever()
