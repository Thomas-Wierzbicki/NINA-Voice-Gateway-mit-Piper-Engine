...
def load_aircraft_database():
    ...
    try:
        with open(CSV_DB_PATH, mode='r', encoding='utf-8', errors='ignore') as f:
            ...
            for row in reader:
                if len(row) >= 3:
                    # KORREKTUR: Zugriff auf Listen-Index statt strip() auf Liste
...
def play_audio_to_radio(is_beacon, content, msg_id):
    ...
    try:
        # VORBEREITUNG: Sprachausgabe berechnen, BEVOR PTT aktiviert wird
        if is_beacon:
            print(f"⏳ Berechne Voice-Teil für Bake (ID {msg_id})...")
            ...
            print(f"⏳ Berechne Sprachalarm (ID {msg_id})...")
            env = os.environ.copy(); env["LD_LIBRARY_PATH"] = BASE_DIR
            subprocess.run([PIPER_BIN, '--model', MODEL_PATH, '--output_file', OUTPUT_WAV], 
                           input=content.encode('utf-8'), env=env, check=True, capture_output=True)
            files_to_play = [OUTPUT_WAV]

        # SENDEN: Erst jetzt PTT einschalten
        ...
        time.sleep(0.6) # Vorlaufzeit

        # Alle Dateien ohne Unterbrechung abspielen
        for f in files_to_play:
            ...
            if len(files_to_play) > 1 and f == BEACON_WAV:
                time.sleep(0.3) # Ganz kurze Pause zwischen Morse und Sprache

        time.sleep(0.3) # Nachlaufzeit
...
def audio_worker():
    while True:
        ...
        play_audio_to_radio(item[0], item[1], item[2])
