...
def find_audio_device(search_name="USB Audio"):
    """Sucht automatisch die ALSA-Kartennummer anhand des Namens."""
    ...
    try:
        ...
        for line in result.stdout.split('\n'):
            if line.startswith('card') and search_name in line:
                # Schneidet die Kartennummer aus der Zeile heraus
...
MQTT_TOPIC   = "#"  # Lauscht auf alle Topics im MQTT-Netzwerk
PTT_PORT     = "/dev/ttyUSB0"
AUDIO_DEVICE = find_audio_device("USB Audio") # Startet die automatische Suche!
...
def on_message(client, userdata, msg):
    # SICHERHEITS-FILTER 1: Ignoriere interne System-Logs von Zigbee2MQTT
    ...
    try:
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                # A: Prüfung auf Text-Nachrichten (Manuelle Durchsagen)
                if "msg" in data:
                    ...
                    # SICHERHEITS-FILTER 2: Falls das 'message'-Feld ein Log ist -> Ignorieren!
                    if "z2m:mqtt" in text_to_speak or "MQTT publish" in text_to_speak:
                        return
                    
                # B: Prüfung auf Temperatur-Sensoren
                    ...
                    # Den Namen des Sensors aus dem MQTT-Topic holen 
                    sensor_name = msg.topic.split("/")[-1].replace("_", " ")
                    
                    # Satz für die Sprachausgabe zusammenbauen
                    text_to_speak = f"Temperatur Information: {sensor_name} meldet {temp} Grad."
                    
                else:
                    # Es ist JSON, hat aber weder Text noch Temperatur
                    return
            else:
                return # JSON-Listen oder einzelne Zahlen ignorieren

        except json.JSONDecodeError:
            # ES IST KEIN JSON -> Blockieren und ignorieren!
            return

        # 3. Wenn wir einen sauberen Text generiert/gefiltert haben -> Senden!
        if text_to_speak:
...
if __name__ == "__main__":
    ...
    if args.ip:
        ...
        try:
            ...
            print(f"   Modus:   Striktes JSON & Sensor-Daten")
